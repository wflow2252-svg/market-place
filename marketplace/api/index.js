...
// 🛡️ SUBDIRECTORY RESOLVER - Target marketplace root
...
try {
    ...
    console.error('[SUBDIR_BOOT_ERROR]:', e);
    app.get('/v1/*', (req, res) => res.status(500).json({ 
        success: false, 
        message: 'SUBDIR_BOOT_ERROR: Missing Backend Components', 
        error: e.message,
        path: e.path
    }));
}
...
app.get('/v1/health', async (req, res) => {
    try {
        ...
        res.status(200).json({ success: true, message: 'API_ELITE_SUBDIR_LIVE' });
    } catch (error) {
        res.status(500).json({ success: false, message: 'DATABASE_OFFLINE', error: error.message });
    }
});

app.get('/v1', (req, res) => res.json({ message: 'API_ELITE_SUBDIR_ACTIVE_V13_SYNCED' }));

// Wildcard Catch
app.all('/v1/*', (req, res) => {
    res.status(404).json({ success: false, message: `SUBDIR_API_NOT_FOUND: ${req.method} ${req.originalUrl}` });
});
